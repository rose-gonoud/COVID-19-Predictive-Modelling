...
async function getCountyUnemploymentData(start_date = "2020-01-01", end_date) {
  if (!end_date) {
  ...
  if (
    ...
    countyUnemploymentData.length == 0 ||
    !lastQueriedStartDate ||
    !lastQueriedEndDate
  ) {
  ...
  //  re-query the api and use those results.
  else if (start_date > lastQueriedStartDate || end_date < lastQueriedEndDate) {
    ...
    console.log(
      "countyUnemploymentData on new start/end date condition",
      countyUnemploymentData
    );
    try {
      storeDataLocally("cachedCountyUnemployment", countyUnemploymentData);
      lastQueriedStartDate = storeDataLocally(
        "lastQueriedStartDate",
        start_date
      );
      lastQueriedEndDate = storeDataLocally("lastQueriedEndDate", end_date);
  ...
  //We loaded from the cache
    ...
    countyUnemploymentData = countyUnemploymentData.filter(
      (unemploymentDatum) => {
        return (
          unemploymentDatum.file_week_ended >= start_date ||
          unemploymentDatum.file_week_ended <= end_date
        );
      }
    );
